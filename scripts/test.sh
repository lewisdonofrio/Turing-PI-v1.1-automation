...
#  Deterministic, out-of-tree safe, nano/vim-safe, ASCII-only.
# ==============================================================
...
cp "$OUT_DIR/arch/arm/boot/zImage" \
   "$STAGING_DIR/boot/zImage-$KERNELRELEASE"

# --------------------------------------------------------------
# Copy DTBs (dot-copy ensures all contents are copied)
# --------------------------------------------------------------
echo "Copying DTBs..."
cp -r "$OUT_DIR/arch/arm/boot/dts/." \
      "$STAGING_DIR/boot/dtbs/"
...
cp -r "$OUT_DIR/lib/modules/$KERNELRELEASE" \
