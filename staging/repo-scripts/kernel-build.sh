...
# ------------------------------------------------------------
# Distcc / pump environment (hostfile-driven)
# ------------------------------------------------------------
HOSTFILE="/home/builder/.distcc/hosts"

if [[ ! -f "$HOSTFILE" ]]; then
    echo "ERROR: Distcc hostfile missing at $HOSTFILE" | tee -a "$LOG"
    exit 1
fi

export DISTCC_HOSTS="$(< "$HOSTFILE")"

echo "DISTCC_HOSTS (from hostfile):" | tee -a "$LOG"
...
# ------------------------------------------------------------
# Build kernel with pump (compile phase)
# ------------------------------------------------------------
echo "Starting distributed kernel build (pump mode)..." | tee -a "$LOG"
pump make -C "$SRC" O="$BUILD" -j"$JOBS" 2>&1 | tee -a "$LOG"
echo "Kernel build (vmlinux + modules) complete." | tee -a "$LOG"

# ------------------------------------------------------------
# Final image stage (non-pump): zImage + DTBs
# ------------------------------------------------------------
echo "Building zImage and DTBs (non-pump stage)..." | tee -a "$LOG"
make -C "$SRC" O="$BUILD" ARCH=arm CROSS_COMPILE=armv7l-unknown-linux-gnueabihf- zImage dtbs 2>&1 | tee -a "$LOG"
echo "zImage and DTBs build complete." | tee -a "$LOG"
