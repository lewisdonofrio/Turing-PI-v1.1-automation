...
if [ "${PYTHON_CHANGED}" -eq 2 ]; then
    echo "==> Python version changed — full rebuild required."
else
...
# ---------------------------------------------------------------------------
# 6. Verify include_server with Python (using pid_file)
# ---------------------------------------------------------------------------
echo "==> Verifying include_server with Python..."
TMPDIR="$(mktemp -d /tmp/pump-restore.XXXXXX)"
PORT="${TMPDIR}/socket"
PID_FILE="${PUMPSAFE_ROOT}/state/include_server.pid"

cd /

echo "   -> Starting include_server.py directly..."
"${PYTHON_BIN}" /usr/lib/python3.13/site-packages/include_server/include_server.py \
    --port "${PORT}" \
    --pid_file "${PID_FILE}" &
sleep 2
...
if [ -f "${PID_FILE}" ]; then
    ...
    if ps -p "${DAEMON_PID}" >/dev/null 2>&1; then
        echo "   -> include_server is running (PID ${DAEMON_PID}), pump-mode backend OK."
        kill "${DAEMON_PID}" 2>/dev/null || true
    else
        echo "ERROR: include_server pid_file exists but process ${DAEMON_PID} is not running."
        exit 1
    fi
else
    echo "ERROR: include_server pid_file was not created at ${PID_FILE}."
    exit 1
fi
